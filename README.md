# fireball - the investment monitoring app
## About
Fireball is an investment app for investors that allows them to plan and track their investments, control dividends, and help them make investment decisions. Works with **T-Invest-Api**.
## Tech stack
- **Backend:** Go
- **Frontend:** Vue 3, TypeScript, Vite
- **Storage:** Redis, PostgreSQL
- **Infrastructure:** Docker, Caddy
- **External API:** T-Invest API (Tinkoff)
## Installation guide
To try app locally clone repo to your machine. App was developed on Ubuntu and MacOS, so Windows is not supported for dev environment. Make sure you you have [go](https://go.dev/doc/install) and [node](https://nodejs.org/en/download) installed on your machine. Setup scripts are written in makefile, so if you for some reason don't have **make** utility, get it installed too
Also, we use image of [Redis](https://hub.docker.com/layers/library/redis/7.0-alpine) and [Postgres](https://hub.docker.com/layers/library/postgres/16-alpine), so [docker](https://docs.docker.com/get-docker/) is necessary for correct work
For db migrations the [goose](https://github.com/pressly/goose) is used

* After cloning repo, create the **.env** file in the root of the repo. Contents example:
```
T_INVEST_URL="https://invest-public-api.tbank.ru"
REDIS_URL="redis://localhost:6379"
REDIS_TTL="24"
PORTFOLIO_CACHE_TTL="3"
CHART_DATA_CACHE_TTL="30"
SESSION_SECRET="your-32-byte-hex-secret-here"
POSTGRES_URL="postgres://fireball:fireball@localhost:5432/fireball?sslmode=disable"
PORT="8080"
READ_TIMEOUT="10"
WRITE_TIMEOUT="10"
IDLE_TIMEOUT="30"
```
* Don't forget to paste correct redis-url and generate and paste 32 bytes secret. You can use `openssl rand -hex 32`
* Default configuration for Postgres is already provided, use your own if you want

* Download dependencies using command `make setup-environment` from the root of the repo
* Enter `make start-deps` for starting required docker containers
* `make migrate` to setup required tables in Postgres
* Type and enter `make build-frontend` to build required assets for frontend
    * For deploying web resources **Vite** is used. It runs on 20s version of Node
* Run `make run-server` to wind up the backend. App will be served on `http://localhost:8080/`
* If you need a development media for real-time edit of frontend code, use `make dev`. Vite will serve app on `http://localhost:3000/`. Don't forget run the server in the previous step, if you want not only frontend page, but fully functional app
* Enter `make stop-deps` for stopping containers

## Architecture
Caddy acts as a reverse proxy, serving static frontend files and forwarding `/api/*` requests to the Go backend.

The backend handles authentication via Redis-backed encrypted sessions. Portfolio and chart data are cached in Redis to minimize calls to the Tinkoff API. Historical candle data is stored persistently in PostgreSQL, with delta fetching to keep it up to date without redundant API calls.

**Request flow:**
Browser → Caddy → Go backend → Redis (session auth) → Redis cache or PostgreSQL → Tinkoff API (on cache miss)

## Feedback
Contact us, authors, for possible suggestions or complaints: 
* [CatSprite](https://github.com/CatSprite-dev)
* [ar3ty](https://github.com/ar3ty)